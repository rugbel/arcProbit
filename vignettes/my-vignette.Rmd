...
The estimated fixed effects and their standard errors are contained in the  fitted object
...
the number of levels for the two random effects equal to `r length(unique(InstEval$s))`  for students and `r length(unique(InstEval$d))` for teachers, which is already out of reach for the existing R libraries for ordinal data models.
 
We fit here a model which has been fitted by others treating the 5-category ordinal response as it were a continuous normal response. The `arcord.fit` function is employed for the task; the starting point is fitting a model for independent data, here made by employing the 
...
## References
- Bellio, R., Ghosh, S., Owen, A. and Varin, C. (2023). Scalable Estimation of Probit Models with Crossed Random Effects. Manuscript in preparation.
